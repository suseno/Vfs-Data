...
class Uko_model extends CI_Model {
	...
	public function get_uko_nama()
	{
		$query = $this->db->query("
			SELECT kode, m,b,u,o,total FROM
			(
			SELECT 
				DISTINCT kanwil_nama_implementor AS kode,
			...
			WHERE uker_implementor NOT IN ('BGT','BSU','CSA','IMS','ING','JDN','KTK','NRA','PVS','SKD','VSI')
			GROUP BY kanwil_nama_implementor
			UNION ALL

				SELECT
					IF (kanwil_nama_implementor = kanwil_nama_implementor,'Grand Total',0) kode,
					SUM(IF(peruntukkan ='MERCHANT',1,0)) m,
					SUM(IF(peruntukkan ='BRILINKS',1,0)) b,
					SUM(IF(peruntukkan ='UKER',1,0)) u,
					SUM(IF(peruntukkan ='OTHERS',1,0)) o,
	...
	public function get_uko_merk()
	{
		$query = $this->db->query("
			...
			WHERE merk_edc IN ('Ingenico','PAX','Verifone', 'Primavista', 'Kartuku', 'Undefined', 'Smartweb', 'Bringin Gigantara', 'PARTNERSHIP BRIZZI', 'Integra', '','OTHER')
			AND uker_implementor NOT IN ('BGT','BSU','CSA','IMS','ING','JDN','KTK','NRA','PVS','SKD','VSI')
			GROUP BY merk_edc
			UNION ALL
	...
	// pengelola search
	public function get_uko_search($tid=NULL, $mid=NULL, $nama_merchant=NULL, $peruntukkan=NULL, $uker_nama_implementor=NULL)
	{
		$ui = array('BGT','BSU','CSA','IMS','ING','JDN','KTK','NRA','PVS','SKD','VSI');
		$this->db->select('*');
		$this->db->from('tb_terminaledc_ref');
